...
//2023.4.27 다시 공부 
// class의 static, const 키워드는 일반적인 static, const 와는 다른 역할을 한다고 생각하는게 맘 편하다.
// 1. 일반적인 class 멤버 변수 - 객체 소멸시 사라지는 변수
// 2. class static 변수 - 프로그램 끝날때까지 존재하고 + 모든 객체가 공유까지함
// 3. class const 변수 - 객체 소멸시 사라지는 변수 + 값 변경못함 초기화 필수.
// 4. class static const 변수 - 프로그램 끝날때까지 존재 + 모든객체 공유 + 값 변경 못함 초기화 필수.
